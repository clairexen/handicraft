...
2. **Gradient-limited sampling.** Each captured input goes through a block-local sampler `n_embd → 2*(n_embd + n_grce) → ReLU → n_grce`. The sampler outputs share weights across timesteps but are detached according to `--context-span` (span 0 keeps gradients, span 1 detaches every step, span N>1 detaches every Nth step).
