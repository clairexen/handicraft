...
Console logs now show `train loss (target)` and `nogrce (target)` where the value outside parentheses is the plain CE loss and the parenthesised number is the aggregate objective (CE + think penalties). When a column is pure CE (for example the `nothink` column), only a single number is shown. Checkpoint histories mirror this convention by storing both `*_loss` (plain CE) and `*_target` entries for every metric, plus `train_wall_seconds` and `unix_time` so downstream analysis scripts can correlate metrics with real time. Sampling/reporting highlight `<think>` as a `❔`; `--no-think` suppresses thinking tokens entirely, `--no-think-prompt` specifically disables thinking-token insertion inside the prompt, and `--think-hard` goes the other way, forcing a thinking token after every prompt token the model mispredicts so you can stress-test completions with dense reasoning bursts.
