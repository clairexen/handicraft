...
- `--import-model some.pt` allows you to seed a new run from an existing checkpoint. Use `--new-layers i,j,...` when growing the layer count (the listed positions stay randomly initialized) or `--trim-model` when shrinking dimensions/layers. The importer copies weights into the overlapping prefix, carries over the total step counter, and writes a fresh `.pt` with empty loss history.
