...
A transformer is a stack of transformer blocks. A transformer block receives a (normaized) N-dimensional vector and produces a (normalized) N-dimensional vector. (Different implementations either put the normalization at the inputs or outputs, usually not both, because that would be redundant.) Inbetween the transformer adds linear components to (or subtracts from) that vector. I'll call those linear components "messages" and I'll call the vector, that is updated by adding and subtracting components, the "channel".

It's easy to see how an N-dimensional vector can carry N messages, if the signals representing those messages are encoded using N orthogonal unit vectors. Each message has a "positive" and a "negative" encoding. A transformer block that just outputs its inputs will trivially not add or subtract anything. A transformer block can easily detect the presence of a given combination of messages: just create the normalized sum of their "positive" encodings, and the dot-product of that vector with the input will be the signal-strenght for that combination of messages. You can also inlcude "negative" encodings for antipatterns, messages or combinations of messages that will never appear together with the combination you want to detect. These antipatterns extend the simpler concept of pure "combinations of (positive) messages" to a richer family of "patterns" by adding contrast. You can add those negative antipattern after normalization. Since they are orthogonal to the other messages, this will leave your detection of the "positive" pattern unchanged, but it will add contrast and will yield zero or negative dot products, if one or more of the anti-patterns is detected. Doing than M times is a NxM matrix multiply. Now we have an M dimensional vector of decoded messages. We run that vector to ReLU, which simply means that we treat any "negative" detections as 0. Thus negative detections have no effect, as one would expect, instead of negative effect. The "effect" is simply what we add to the channel. We take the all-positive vector after ReLU and perform a MxN matrix multiply, that assigns each neuron a "message" to add (or subtract) from the channel.

The normalization step dampens all the old components. If they are not needed downstream, backprop will make the new components a bit louder. If they are still needed downstream, backprop will not only tune down the new components a bit, so one can still "hear" the previous block more clearly downstream, but the network will learn to amplify all components that are needed downstream unprocessed. This mechanism creates a smooth surface that drastically simplifies and speeds up the process of learning basic principles in a deep network quickly and then gradually use more and more of the available compute resources in each layer to create a richer and richer model.
