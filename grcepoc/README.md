...
Think of a Transformer stack as a long sequence of “mix → detect → remix” stages operating on a shared channel. Each block consumes a normalized N-dimensional vector, applies a few carefully shaped linear projections, injects nonlinearities (ReLU/GELU), and hands back another normalized N-dimensional vector. Whether the normalization is pre-norm (LayerNorm before attention/MLP) or post-norm (after) is an implementation choice, but the effect is the same: keep the channel stable so every block receives a comparable signal-to-noise ratio.

### Messages riding on an orthogonal basis
An N-dimensional vector can simultaneously carry N independent “messages” if each one aligns with an orthogonal basis vector. In practice the basis is not literally orthogonal—the model learns whatever mixture best represents the training distribution—but it helps to imagine that each neuron owns a direction in embedding space. When a block wants to detect a pattern, it creates a probe vector aligned with the relevant positive directions and computes a dot product with the incoming channel. The result is a scalar score saying “how loudly is this pattern currently active?”
