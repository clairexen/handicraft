...
- `--reward-relu SCALE` enables an experimental auxiliary update that periodically boosts frequently active neurons from well-predicted positions and nudges the biases of inactive neurons from poorly predicted positions. `SCALE` is the tiny multiplier (default 0 disables the feature), and updates fire mid-cycle and at the end so they never require persisting extra state.
