...
4. **Bias injection.** At the next position every block applies its own generator `n_grce → 2*(n_embd+n_grce)/n_layer → ReLU → Dropout → n_embd`. The generated biases are added only to the newest token row of each block input, so the rest of the sequence remains untouched while the context acts as an additive steering signal.
