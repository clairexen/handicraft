...
2. **Gradient-limited sampling.** Each block input goes through LayerNorm; all `n_layer` normalized vectors are concatenated (shape `n_layer * n_embd`) and run through a single sampler `→ 2*(n_embd+n_grce)/n_layer → ReLU → Dropout → n_grce`. The result is detached according to `--context-span` (span 0 keeps gradients, span 1 detaches every step, span N>1 detaches every Nth step).
3. **Context propagation.** The sampler output is squashed with `tanh`, LayerNorm'ed once, and becomes the single context vector for the next position—nothing else persists across time.
4. **Bias injection.** The context vector is mapped back through `n_grce → 2*(n_embd+n_grce)/n_layer → ReLU → Dropout → (n_layer * n_embd)`, then split into `n_layer` slices; each slice feeds the newest token row of its block input, so the rest of the sequence remains untouched while the context acts as an additive steering signal.
