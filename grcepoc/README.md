...
`plain` mirrors the "pure Transformer" special row by disabling both GRCE and XCTX entirely so the core model has to fend for itself. `normal` reuses the baseline (context-enabled, attention-enabled) evaluation. The `encode` column reruns that baseline with the causal mask removed so every token can attend bidirectionally. The other three diagnostics—`noctx`, `noatt`, and `none`—disable the XCTX channel, attention, or both while leaving GRCE untouched, letting you verify that the recurrent paths continue to add value even when other shortcuts are compromised.
...
- **Model evaluation:** Each evaluation pass prints three loss groups when thinking is enabled: the main columns (`target`, `noprev`) described above, the six-column stress test (`plain`, `normal`, `noctx`, `noatt`, `none`, `encode`), and the `think/2x/3x` trio. The new `plain` column runs the entire batch as a pure Transformer (both GRCE and XCTX disabled), `normal` continues to track the baseline context-enabled run, and `encode` repeats that baseline with bidirectional (encoder-style) attention so you can gauge how much the causal mask costs. The last set measures how well the learned thinking policy behaves when we (a) insert `<think>` tokens exactly where the base model predicted them, (b) force one `<think>` per token, or (c) force two `<think>` tokens per token. For those metrics we only score the final `<think>` position in each sequence (after masking out earlier `<think>` logits) and weight each contribution by the number of inserted steps so longer “reasoning chains” remain comparable to shorter ones. When thinking is disabled the third group is omitted.
