...
-
- **Architektur:** Standardmäßig `n_layer=8`, `n_head=8`, `n_embd=192`, `n_grce=96` und `block_size=64` (vergleichsweise kompakt; GPT-2 base liegt bei 12/12/768/1024). Der explizite GRCE-Kanal nutzt `n_grce` und verarbeitet den aktuellen Token-Input (inkl. Kontext-Bias) plus **alle FFN-Ausgänge der vorherigen Position**. Die Geometrie folgt `(n_layer+1)*n_embd → 4·n_embd → n_grce → 4·n_embd → n_embd`; hier endet der Gradientenfluss (Stop-Grad), sodass das rekurrente Signal als explizit kontrollierte Überlagerung geführt wird.
