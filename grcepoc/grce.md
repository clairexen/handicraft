...
-
- **Architektur:** GPT-ähnliches Decoder-Modell mit 8 Layern, 8 Köpfen, 512 Embedding-Dimensionen (vgl. GPT-2 base mit 12/12/768). Der explizite GRCE-Kanal bleibt wie beschrieben und nutzt `context_dim = n_embd`; sein Schreib-FFN verarbeitet den aktuellen Token-Input (inkl. Kontext-Bias) plus **alle FFN-Ausgänge der vorherigen Position**. Die Geometrie folgt `(n_layer+1)*n_embd → 4·n_embd → n_grce → 4·n_embd → n_embd`; hier endet der Gradientenfluss (Stop-Grad), sodass das rekurrente Signal als explizit kontrollierte Überlagerung geführt wird.
