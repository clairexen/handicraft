...
- **Token-Caches:** Nach dem ersten Durchlauf landen die tokenisierten `train`/`test`-Tensoren als `model/<data>_tokens_<split>_<chars>_<vocab>.pt` auf der Platte. Existieren diese Dateien (sowie das Tokenizer-JSON), kann das Skript ohne die Original-`data/*.asc` Dateien starten – ideal für schnelle Iterationen oder Archivierung.
