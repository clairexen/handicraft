...
-
- **Architektur:** Standardmäßig `n_layer=8`, `n_head=8`, `n_embd=192`, `n_grce=96` und `block_size=64` (vergleichsweise kompakt; GPT-2 base liegt bei 12/12/768/1024). Der explizite GRCE-Kanal bildet die `stack vector` genannte Verkettung aus aktuellem Token-Input plus **allen FFN-Ausgängen der vorherigen Position** (`(n_layer+1)*n_embd`). Jede der `(n_layer+1)` Komponenten erhält zunächst eine eigene LayerNorm, bevor sie in das Writer-MLP `(stack → 4·(n_embd+n_grce) → GELU → 4·(n_embd+n_grce) → GELU → n_grce → LayerNorm)` eingeht; dieses normalisierte `n_grce`-Signal definieren wir als `context`. Für jeden Block existiert anschließend ein Bias-Generator `context → 4·n_grce → GELU → 4·n_embd → GELU → n_embd`, dessen Ergebnis nur auf das neu angefügte Token im entsprechenden Block addiert wird. Der Gradientenfluss wird ausschließlich an der `stack vector` abgeschnitten; sowohl `context` als auch alle Bias-Pfade bleiben differentiabel.
