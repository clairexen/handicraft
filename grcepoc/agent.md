...
- **GRCE geometry:** `n_layer * [LayerNorm → n_embd → 2*(n_embd+n_grce) → ReLU → Dropout → n_grce]` averaged/tanh-clamped into the context vector, then `n_layer * [LayerNorm → n_grce → 2*(n_embd+n_grce) → ReLU → Dropout → n_embd]` for the per-layer biases.
