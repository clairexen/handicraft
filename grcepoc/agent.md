...
- **GRCE geometry:** `(n_layer+1)*n_embd → 4*n_embd → n_grce → 4*n_embd → n_embd` with gradient limiting applied at the concatenation of token input + previous FFN outputs.
