...
- **GRCE geometry:** `n_layer * [LayerNorm → n_embd → 4*n_grce → ReLU → Dropout → n_grce]` averaged/tanh-clamped into the context vector, LayerNorm'ed again post-avg, then `n_layer * [n_grce → 4*n_grce → ReLU → Dropout → n_embd]` for the per-layer biases.
