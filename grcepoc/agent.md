...
- **GRCE geometry:** `n_layer * [n_embd → 4*n_grce → ReLU → Dropout → n_grce]` averaged/tanh-clamped into the context vector, then `n_layer * [n_grce → 4*n_grce → ReLU → Dropout → n_embd]` for the per-layer biases. A single LayerNorm sits at `--context-norm` (pre/per/post).
