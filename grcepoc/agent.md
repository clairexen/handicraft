...
- **GRCE geometry:** `n_layer * [LayerNorm → n_embd → 2*(n_embd+n_grce)/n_layer → ReLU → Dropout → n_grce]` averaged/tanh-clamped into the context vector, LayerNorm'ed again post-avg, then `n_layer * [n_grce → 2*(n_embd+n_grce)/n_layer → ReLU → Dropout → n_embd]` for the per-layer biases.
