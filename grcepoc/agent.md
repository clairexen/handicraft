...
- **GRCE geometry:** `n_layer * [LayerNorm → n_embd → 4*n_grce → ReLU → Dropout → n_grce]` averaged/tanh-clamped into the context vector, then `n_layer * [LayerNorm → n_grce → 4*n_grce → ReLU → Dropout → n_embd]` for the per-layer biases.
...
- **Context norm switch:** `--context-norm` controls whether LayerNorm is applied before aggregation (`pre`), per-layer after sampling (`per`), or once after averaging (`post`).
