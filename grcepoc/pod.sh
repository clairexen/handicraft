...
pod_init() {
        run_ssh "set -ex; ln -sf /usr/share/zoneinfo/Europe/Vienna /etc/localtime; mkdir -p $REMOTE_DIR/data $REMOTE_DIR/model; echo $POD_NAME > $REMOTE_DIR/.podname; apt update; apt install -y rsync; pip install tokenizers transformers nvidia-ml-py"
}
