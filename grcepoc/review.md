...
## 1. Overview of GRCE
The Gradient-limited Recurrent Context Encoding (GRCE) channel augments a decoder-only Transformer with a narrow recurrent path. For every position, the inputs to all Transformer blocks are projected through shared sampler MLPs (`n_embd → 2*(n_embd+n_grce) → ReLU → n_grce`). The resulting vectors are summed, optionally detached according to `--context-span`, and forwarded as the sole context state. At the next position, block-specific generators (`n_grce → 2*(n_embd+n_grce) → ReLU → n_embd`) inject additive biases only into the newest token row. The design targets two behaviors: long-lived components (style/role switches) and short-lived controllers (grammar trackers), while keeping the recurrent state explicitly bounded and easy to throttle.
