...
def help():
    ...
    print()
    print(f"Play and Autoplay Options (they are exclusive, use max 1):")
    ...
    print(f"  The state arg initializes the internal state of the Wordle-engine")
    print(f"  to any state that can be reached by any series of Worlde guesses")
    print(f"  and responses. The special state arg '/' represents the initial")
    print(f"  state of the engine before the first guess. Any other state is")
    print(f"  encoded using the following syntax:")
    print(f"    /<ch1>/<ch2>/<ch3>/<ch4>/<ch5>/<usechars><stopchars>")
    print()
    print(f"  <ch1>..<ch5>  specify the allowed letters at the position")
    print(f"                upper case string = allowed letters")
    print(f"                lower case string = forbidden letters")
    print(f"                empty string = any letter is allowed")
    print()
    print(f"  <usechars>    uppercase string that specifies letters that must be used")
    print(f"                for example, if you ever got two non-gray As in any guess,")
    print(f"                then this string will contain the char 'A' twice")
    print()
    print(f"  <stopchars>   lowercase string that specifies letters that must not be")
    print(f"                used more often, than indicated in <usechars> above.")
    print(f"                Whenever a letter was displayed on gray background for any")
    print(f"                guess in a session, that letter is added to <stopchars>.")
    print()
    print(f"  Example State Arg: /u//i/du/D/DIUaefnpr")
    print()
    print(f"  Note that State Args always start with a '/' character.")
    print()
    print(f"Guess Args (encode a single guess and the results it produced):")
    print(f"  Guess Args consist of a (5 letter lowercase) guess, followed by a slash,")
    print(f"  followed by a 5-chars result string. The result string contains a '_' or")
    print(f"  '.' in all positions with gray background, replicates the lowercase letter")
    print(f"  from the same position in the guess for all letters displayed on yellow")
    print(f"  background, and the uppercase version of that letter for all letters")
    print(f"  displayed on green background.")
    print()
    print(f"  Note that Guess Args always contain a '/' character in the 6th position.")
    print()
    print(f"Query Args (are simple guess words wthout results attached to them):")
    print(f"  Query Args are just (5 letter lowercase) dictionary words. Adding one or")
    print(f"  more Query Args at the end of the command line, will print various infos")
    print(f"  about those words as guesses in the context of the current engine state.")
    print()
    print(f"  When a secret word has been set (with -p, -P, -w, -a, or -A), then args")
    print(f"  that are simple dictionary words are automatically expanded to Guess Args")
    print(f"  using the current secret word, and are then interpreted as Guess Args")
    print(f"  and not Query Args.")
    print()
    sys.exit(1)
